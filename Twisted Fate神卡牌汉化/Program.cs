...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("鍗＄墝", "TwistedFate", true);

            var TargetSelectorMenu = new Menu("鐩爣閫夋嫨", "Target Selector");
            SimpleTs.AddToMenu(TargetSelectorMenu);
            Config.AddSubMenu(TargetSelectorMenu);

            var SowMenu = new Menu("璧扮爫閫夐」", "Orbwalking");
            SOW = new Orbwalking.Orbwalker(SowMenu);
            Config.AddSubMenu(SowMenu);

            /* Q */
            var q = new Menu("Q璁剧疆", "Q");
            q.AddItem(new MenuItem("AutoQI", "鑷姩Q1").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "鑷姩Q2").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "蹇嵎閿").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W璁剧疆", "W");
            w.AddItem(
            ...
            var r = new Menu("R璁剧疆", "R");
            r.AddItem(new MenuItem("AutoY", "鑷姩榛勭墝").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("鏉傞」", "Misc");
            misc.AddItem(new MenuItem("PingLH", "娈嬭鎻愰啋").SetValue(true));
            ...
            /*Drawing*/
            var Drawings = new Menu("鏄剧ず璁剧疆", "Drawings");
            Drawings.AddItem(new MenuItem("Qcircle", "Q鑼冨洿").SetValue(new Circle(true, Color.FromArgb(100,255,0,255))));
            Drawings.AddItem(new MenuItem("Rcircle", "R鑼冨洿").SetValue(new Circle(true, Color.FromArgb(100, 255, 255, 255))));
            Drawings.AddItem(new MenuItem("Rcircle2", "R鑼冨洿(灏忓湴鍥)").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
