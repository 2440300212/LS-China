...
namespace EasyEzreal
{
    class EazyEzreal : Champion
    {
        ...
        protected override void InitializeMenu()
        {
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_w", "浣跨敤 W").SetValue(true));

            Menu.AddSubMenu(new Menu("楠氭壈", "Harass"));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_q", "浣跨敤 Q").SetValue(true));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_w", "浣跨敤 W").SetValue(false));

            Menu.AddSubMenu(new Menu("鑷姩", "Auto"));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_q", "浣跨敤 Q").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_w", "浣跨敤 W").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_r", "浣跨敤 R").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_minrange", "R|鏈€灏弢鑼冨洿").SetValue(new Slider(1050, 0, 1500)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_maxrange", "R|鏈€澶鑼冨洿").SetValue(new Slider(3000, 1500, 5000)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_manaE", "淇濊瘉 E 钃濋噺").SetValue(true));
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_w", "W 鑼冨洿").SetValue(new Circle(true, Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_rdamage", "澶ф嫑浼ゅ").SetValue(true));
        }
