...
namespace LeagueSharp.Common
{
    ...
    public static class SimpleTs
    {
        ...
        {
            _config = Config;
            Config.AddItem(new MenuItem("FocusSelected", "閲嶇偣 閫夋嫨 鐩爣").SetShared().SetValue(true));
            Config.AddItem(
                new MenuItem("SelTColor", "鐩爣 棰滆壊").SetShared()
                    .SetValue(new Circle(true, System.Drawing.Color.Red)));
            Config.AddItem(new MenuItem("Sep", "").SetShared());
            var autoPriorityItem = new MenuItem("AutoPriority", "鑷姩|瀹夋帓|浼樺厛绾").SetShared().SetValue(false);
